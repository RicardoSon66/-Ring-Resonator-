...
하지만 단위시간 1000일때는 Decay로 인해 링 내부의 에너지가 완전 소멸이 되는걸 관측이 안되는것을 확인하였습니다.    
이를 다른 방면으로 생각을 해보면 이 Resonators는 단위시간 1000 동안의 Store 성능을 시각적으로 검증이 가능하다는것을 해당 시뮬레이션으로 통해 확인 하였습니다.  
<br>
...
| 비교 항목 | **Pulse Wave** | **Continuous Wave** |
| :--- | :--- | :--- |
| **에너지 전달 특성** | 빛이 링에 머물다가 서서히 빠져나감 | 지속적인 에너지 유입으로 내부 에너지가 포화됨 |
| **관측 현상** | 남아있는 빛이 사라지는 과정 | 간섭으로 인해 빛이 강해지거나 약해지는 과정 |
| **물리적 의미** | 공진기의 **에너지 저장 능력** 확인 | 공진 파장에 따른 **필터링 및 변조 특성** 확인 |
| **주요 메커니즘** | 에반에센트 결합에 의한 에너지 재결합 | 입사광과 순환광 사이의 위상 정합(Phase Matching) |
<br>
