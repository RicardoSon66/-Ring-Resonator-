...
Resonator = gf.components.ring_single(
    gap=0.2,
    radius=5.0,
    length_x=0.0,
    length_y=0.0
)
Resonator = gf.add_padding_container(
    Resonator,
    default=0,
    top=3,
    bottom=3,
...
h = mp.Harminv(mp.Ez, mp.Vector3(5.7, 0, 0), Source_f, Source_width)

print('Ring Resonator 시뮬레이션 시작...')
sim.run(
    mp.after_sources(h),
...
ring_flux = np.array(mp.get_fluxes(flux_ring_mon))
ref_flux = np.load("ref_flux.npy")
freqs = np.load("ref_freqs.npy")

# 5. 정규화된 S21 (dB) 계산
s21 = 10 * np.log10(ring_flux / ref_flux)
